...
class SubopdController extends Controller
{
    protected $presensi;
    protected $subopd;
    public function __construct(PresensiService $presensiService, SubOpdService $subOpdService)
    {
    ...
    {
        if ($request->ajax()) {
            $data['table'] = $this->subopd->Query()->where('opd_id', Auth::user()->opd_id)->get();
            return view('subopd._data_table_subopd', $data);
        }
        $data['title'] = 'Titik Kumpul';
        return view('subopd.index', $data);
    }
