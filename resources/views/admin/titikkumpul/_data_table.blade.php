    ...
    <tbody>
        @forelse ($table as $key => $tb)
            <tr>
                ...
                <td>{{ $tb->nama_lokasi }}</td>
                <td>{{ $tb->lat ?? '-' }}</td>
                <td>{{ $tb->long ?? '-' }}</td>
                <td>
                    <button class="btn {{ $tb->status == 'true' ? 'btn-primary' : 'btn-danger' }} btn-sm"
                        onclick="updateStatus({{ $tb->id }}, event);">{{ $tb->status == 'true' ? 'Aktif' : 'Tidak Aktif' }}</button>
                </td>
                <td><a href="/admin/titik_kumpul/show/{{ $tb->id }}" class="btn btn-primary btn-sm">Lihat</a></td>
            </tr>
        @empty
